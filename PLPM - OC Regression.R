...
RoughDataUtilisation <- function(data.set, perc) {
  temp.int <- (perc*nrow(data.set))/(237*100)
  return(temp.int)
}
#assign("outward.code", data.frame())
#outward.code <- GetSectors(outward.code.choice)
data.util <- RoughDataUtilisation(outward.code, 5)
md.df <- GetSectorMedians(outward.code, data.util)

outward.code2 <- outward.code

temp.result <- split(outward.code2,outward.code2$post_code)[[postcode.choice]]
